...
float4 main_0(PS_INPUT Input) : COLOR
{
  ...
  float4 c1 = Tex1 * Alpha.r * Rd;
  float4 c2 = Tex2 * Alpha.g * Gd;
  float4 c3 = Tex3 * Alpha.b * Bd;
  c1.w = 1.0f;
  c2.w = 1.0f;
  c3.w = 1.0f;


  return (c1 + c2 + c3 + brush * 0.4f);
};
...
VS1_OUTPUT WaveShader_Pass_1_Vertex_Shader_vs_main( VS1_INPUT Input )
{
   VS1_OUTPUT Output;
...
technique Shader
{
    ...
    pass Pass_1
    {
       ...
       // ZWRITEENABLE = TRUE;
        ...
        VertexShader = compile vs_2_0 WaveShader_Pass_1_Vertex_Shader_vs_main();
        PixelShader = compile ps_2_0 WaveShader_Pass_1_Pixel_Shader_ps_main();
    }   
