...
class cMapDataManager
{
	...
	SYNTHESIZE_PASS_BY_REF(string, m_sFilePath, FilePath);						    // x파일 파일경로
	SYNTHESIZE_PASS_BY_REF(string, m_sFileName, FileName);					    	// x파일 이름 .x 로 끝남
    SYNTHESIZE_PASS_BY_REF_NO_SET(E_OBJECT_BUTTON_STATE, m_eObjectButtonState, ObjectButtonState);  // 오브젝트 탭 버튼 클릭 상태 
    // BLOCK_EDIT
