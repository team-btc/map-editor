...
BOOL cTerrainTab::OnInitDialog()
{
    ...
    m_pEditHeightSliderCtl->SetPos((int)m_fEditHeight);		// 위치 설정
    ...
    m_pEditHeightSliderCtl->SetPageSize(10);			// 증가 크기(PgUP,Dn 키나 슬라이더 몸동을 클릭하여 움직일 때)
                                                        ...
                                                        // 편집 높이 출력
    SetDlgItemInt(IDC_EDIT_HEIGHT_EDI, (UINT)m_fEditHeight);
...
void cTerrainTab::OnChangeEditHeightEditer()
{
    ...
    // 에디터에 입력 된 값  가져오기
    m_fEditHeight = (float)GetDlgItemInt(IDC_EDIT_HEIGHT_EDI);
...
void cTerrainTab::OnCustomDrawEditHeightSlider(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    // 편집 높이 넣기
    m_fEditHeight = (float)m_pEditHeightSliderCtl->GetPos();
